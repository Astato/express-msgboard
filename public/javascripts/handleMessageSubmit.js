  ...
  if (isPrivate && issuer) {
    if (isFriendChatSelected[0] === issuer) {
      ...
      Array.from(friendlist).forEach((friend) => {
        if (friend.childNodes[0].textContent === issuer) {
          const messageIndicator = friend.childNodes[1].textContent;
          const checkForMessagesCount = /[0-9]/g;
          const unreadMessages = messageIndicator.match(checkForMessagesCount);
          if (unreadMessages) {
            return (friend.childNodes[1].textContent =
              "New (" + (Number(unreadMessages[0]) + 1) + ")");
          }
          friend.childNodes[1].textContent = "New (1)";
          return;
        }
      });
    }
...
socket.on("New Message", (message) => {
  if (!globalChat || globalChat.className === "focused") {
    return updateMessagesUI(message, true);
  } else {
    return (globalChat.textContent =
      globalChat.textContent + " New (" + newGlobalMessageCount + 1 + ")");
  }
});
...
function updateConnectedFriends(connectedUser) {
  console.log(connectedUser, "FUNCTION CALL");
  Array.from(friendlist).forEach((friend) => {
    if (friend.textContent === connectedUser) {
      console.log("CONNECTED", friend.textContent, connectedUser);
...
Array.from(friendlist).forEach((element) => {
  element.addEventListener("click", async () => {
    ...
    isFriendChatSelected = [clickedUser, element.id];
    await getFriendChat(username, clickedUser)
      .then(() => socket.emit("Selected Chat", clickedUser, username))
